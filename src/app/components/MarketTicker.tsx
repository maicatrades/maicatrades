...
type SessionLabel = "PRE" | "REGULAR" | "AH";
...
export default function MarketTicker() {
  ...
  useEffect(() => {
    ...
    async function fetchTickerData() {
      try {
        ...
        if (!response.ok) throw new Error("Unable to load market ticker");

        const result = await response.json();

        if (!result.success || !Array.isArray(result.data)) {
          throw new Error(result.error || "Invalid market ticker response");
        }
        ...
        result.data.forEach((ticker: MarketTickerItem) => {
          const previousPrice = previousPrices.current[ticker.symbol];

          if (previousPrice !== undefined && ticker.price !== null) {
            nextFlashState[ticker.symbol] =
        ...
        setLastUpdated(
          new Date(result.updatedAt ?? Date.now()).toLocaleTimeString("en-US", {
            hour: "numeric",
            minute: "2-digit",
            second: "2-digit",
          }),
    ...
    fetchTickerData();
    const interval = window.setInterval(fetchTickerData, 60_000);
  ...
  const scrollingTickerData = [...tickerData, ...tickerData];
        ...
        <div className="relative min-w-0 flex-1 overflow-hidden">
          ...
          <div className="market-ticker-track flex min-w-max items-stretch group-hover/ticker:[animation-play-state:paused]">
            {scrollingTickerData.map((ticker, index) => {
              ...
              const negative =
                ticker.changePercent !== null && ticker.changePercent < 0;
              const flashDirection = flashState[ticker.symbol];
              const flashClass =
                flashDirection === "up"
                  ? "bg-emerald-400/[0.12]"
                  : flashDirection === "down"
                ...
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {ticker.sessionLabel && (
                        <span className="rounded border border-sky-500/30 bg-sky-500/10 px-1 py-0.5 text-[8px] font-bold tracking-wide text-sky-400">
                          {ticker.sessionLabel}
                        </span>
                      )}
                    </div>
                    <p className="mt-0.5 truncate text-[10px] text-slate-500">
                      {ticker.symbol === "VIX" ? "Volatility Index" : "Market ETF"}
                    </p>
                  </div>
